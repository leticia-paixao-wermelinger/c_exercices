...
void broadcastMsg(int *clients, char *msg, enum msgType type, int sender_fd)
{
	...
	{
		// ALTERADO: broadcastMsg nao tenta mais extrair mensagens.
		// Ela recebe uma linha completa ja separada por checkPendingMessages().
...
int addNewClient(int connfd, int *clients, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		...
		{
			clients[i] = connfd;
			// ALTERADO: cada cliente novo comeca sem texto parcial guardado.
			pending[connfd] = 0;
...
int checkPendingMessages(fd_set *readfds, int *clients, fd_set *allfds, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		...
		if (FD_ISSET(fd, readfds))
		{
			...
			if (n <= 0)
			{
					// ALTERADO: removeClient tambem limpa pending[fd].
			...
			// ALTERADO: guardo o chunk atual junto do que ja tinha sobrado.
			pending[fd] = str_join(pending[fd], buffer);
			if (pending[fd] == 0)
				fatalError();
			// ALTERADO: so mando para os outros clientes linhas completas.
			while ((status = extract_message(&pending[fd], &line)) > 0)
			{
			...
			if (status == -1)
				fatalError();
			// ALTERADO: se extract_message deixou uma string vazia, libero.
			if (pending[fd] != 0 && pending[fd][0] == '\0')
			{
				free(pending[fd]);
				pending[fd] = 0;
			}
...
void removeClient(int fd, int *clients, fd_set *allfds, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		if (clients[i] == fd)
		{
			...
			clients[i] = -1;
			// ALTERADO: descarto fragmentos sem '\n' ao desconectar.
			free(pending[fd]);
			pending[fd] = 0;
...
void	main_loop(int sockfd)
{
	...
	// ALTERADO: um buffer persistente por fd para juntar chunks de recv().
	...
	{
		clients[i] = -1;
		// ALTERADO: nenhum fd tem fragmento pendente no inicio.
		pending[i] = 0;
	}
