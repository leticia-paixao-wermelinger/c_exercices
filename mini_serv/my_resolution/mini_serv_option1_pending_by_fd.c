...
void removeClient(int fd, int *clients, fd_set *allfds, char **pending);
...
void broadcastMsg(int *clients, char *msg, enum msgType type, int sender_fd, char **pending)
{
	char	*full_msg;
	char	tmp[128];
	char	*line;
	...
	{
		// ALTERADO: antes eu copiava apenas o chunk atual recebido por recv().
		// Agora eu acumulo esse chunk no buffer persistente do fd do cliente.
		pending[sender_fd] = str_join(pending[sender_fd], msg);
		if (pending[sender_fd] == 0)
			fatalError();
		// ALTERADO: extract_message trabalha no histórico inteiro do cliente,
		// então uma linha quebrada entre dois recv() continua inteira.
		while ((status = extract_message(&pending[sender_fd], &line)) > 0)
		{
		...
		if (status == -1)
			fatalError();
		// ALTERADO: se não sobrou nada depois do '\n', libero o buffer vazio.
		if (pending[sender_fd] != 0 && pending[sender_fd][0] == '\0')
		{
			free(pending[sender_fd]);
			pending[sender_fd] = 0;
		}
	}
	if (full_msg == 0)
...
int addNewClient(int connfd, int *clients, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		...
		{
			clients[i] = connfd;
			// ALTERADO: inicializo o buffer parcial desse fd.
			pending[connfd] = 0;
			broadcastMsg(clients, NULL, ARRIVED, connfd, pending);
			return 0;
		}
	}
...
int	identifyNewClient(fd_set *readfds, fd_set *allfds, int sockfd, int *clients, int *maxfd, char **pending)
{
	...
	if (FD_ISSET(sockfd, readfds))
	{
		...
		if (connfd >= 0)
		{
			if (addNewClient(connfd, clients, pending) == -1)
				return (-1);
...
int checkPendingMessages(fd_set *readfds, int *clients, fd_set *allfds, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		...
		if (FD_ISSET(fd, readfds))
		{
			...
			if (n <= 0)
			{
					// ALTERADO: removeClient precisa liberar pending[fd].
					removeClient(fd, clients, allfds, pending);
				continue;
			}

			// Caso contrário, processa a mensagem recebida (n > 0)
			buffer[n] = '\0'; // Adiciona um terminador de string ao final da mensagem
			// ALTERADO: o broadcast agora junta este chunk ao pending[fd]
			// antes de extrair linhas completas.
			broadcastMsg(clients, buffer, MESSAGE, fd, pending);
		}
	}
	return (0);
}
...
void removeClient(int fd, int *clients, fd_set *allfds, char **pending)
{
	...
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		if (clients[i] == fd)
		{
			...
			clients[i] = -1;
			// ALTERADO: descarto qualquer pedaço sem '\n' que ficou pendente.
			free(pending[fd]);
			pending[fd] = 0;
			broadcastMsg(clients, NULL, LEFT, fd, pending);
			return ;
		}
	}
}

void clearAllFds(fd_set *allfds, int *clients, int sockfd, char **pending)
{
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		if (clients[i] != -1)
			removeClient(clients[i], clients, allfds, pending);
	}
	close(sockfd);
}
...
void	main_loop(int sockfd)
{
	...
	// ALTERADO: um buffer persistente por fd para guardar mensagens incompletas.
	char *pending[FD_SETSIZE];

	// Inicializa o array de file descriptors dos clientes com -1 (indicando que estão disponíveis)
	for (int i = 0; i < FD_SETSIZE; i++)
	{
		clients[i] = -1;
		// ALTERADO: cada fd começa sem pedaços pendentes.
		pending[i] = 0;
	}
	...
	while (g_should_stop == 0)
	{
		...
		if (identifyNewClient(&readfds, &allfds, sockfd, clients, &maxfd, pending) == -1)
			break;
		
		if (checkPendingMessages(&readfds, clients, &allfds, pending) == -1)
			break;
	}

	clearAllFds(&allfds, clients, sockfd, pending);
}
